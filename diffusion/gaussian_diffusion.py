...
class GaussianDiffusion:
    ...
    def _loss_history_update(self, ts, losses, loss_masks, training_step): #v5
        ...
        """
        all_losses = losses.detach().cpu().numpy()  # BxS
        all_loss_masks = loss_masks.detach().cpu().numpy()  # BxS
        all_ts = ts.detach().cpu().numpy()  # B
    ...
    def training_losses(self, model, training_step, t, model_kwargs=None, noise=None):
        ...
        if 'loss_mask' in model_kwargs:
            ...
            loss_mask = None
        real_model = model.model.module if hasattr(model.model, "module") else model.model
        # x_start_mean = real_model.get_embeds(input_ids)
        x_start_mean = input_ids
        ...
        with th.no_grad():
            ...
            temp_ids = input_ids[:, :, 0].squeeze(1)
            temp_ids[:, 1:] = self.pad_tok_id
            _loss_log[temp_ids==self.pad_tok_id] = 0
            # _loss_log[input_ids==self.pad_tok_id] = 0
            # self._loss_history_update(t, _loss_log, input_ids!=self.pad_tok_id, training_step)
            self._loss_history_update(t, _loss_log, temp_ids != self.pad_tok_id, training_step)
    ...
    def token_discrete_loss(self, x_t, get_logits, input_ids, mask=None):
        decoder_nll = self.float_label_loss(x_t, input_ids)
        if mask is not None:
    ...
    def float_label_loss(self, logits, labels, ignore_index=-100):
        """
        logits: float tensor, shape [B, L, V]
        labels: float tensor, shape [B, L, V], float labels (probabilities)
        """
        # mask: 用于忽略 pad token
        mask = labels.sum(dim=-1) > 0  # 只对非pad token计算 loss
        loss = F.mse_loss(logits, labels, reduction='none')
        loss = loss.mean(dim=-1)  # sum over vocab dim -> [B, L]
        loss = loss * mask.float()  # mask掉pad位置
        return loss  # shape: [B, L], 每个位置的 loss
