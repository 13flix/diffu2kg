...
class TransformerNetModel_encoder_decoder(nn.Module):
    ...
    def __init__(
        ...
        if config is None:
            config = AutoConfig.from_pretrained(os.path.join(os.path.dirname(__file__), "pretrain", "bart-base"), local_files_only=True)
        ...
        if not self.init_pretrained:
            ...
            self.embedding_dim = self.config.d_model // 4 #self.config.d_model // 4
        ...
        if self.load_ckpt is not None:
            if self.load_ckpt and os.path.exists(self.load_ckpt):
                self.load_weight(self.load_ckpt)
            else:
                print(f"[Info] No checkpoint loaded, start from scratch.")

    def get_embeds(self, input_ids):
        # return self.input_transformers.decoder.embed_tokens(input_ids) * self.embed_scale
        return input_ids * self.embed_scale

    def load_weight(self, path):

        self.load_state_dict(torch.load(path))
        print(f'weight initialize from {path}')

    def build_xstart_predictor(self):
        if self.init_pretrained:
            local_path = os.path.join(os.path.dirname(__file__), "pretrain", "bart-base")
            temp_bart = BartModel.from_pretrained(local_path, config=self.config, local_files_only=True)
    ...
    def forward_encoder(self, 
        ...
        encoder_hidden_states = self.input_transformers(
            ...
            inputs_embeds = self.input_up_proj_enc(input_ids),
    ...
    def forward(self, 
        ...
        input_trans_hidden_states = self.input_transformers(
            input_ids=None,
            attention_mask=attention_mask,
            inputs_embeds=self.input_up_proj_enc(input_ids) if input_ids is not None else None,
            decoder_input_ids=None,
            decoder_inputs_embeds=emb_inputs,
            decoder_attention_mask=decoder_attention_mask,
